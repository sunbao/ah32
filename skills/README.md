...
当前版本：prompt（后端）+ tools（客户端脚本工具描述）。
注意：后端已禁用执行 `skill.json.tools` 对应的 `scripts/*.py`（迁移为前端 JS 执行）。
...
如果技能需要“确定性能力”（例如：解析结构化文本、生成检查清单、版式/占位映射），可以在 `skill.json.tools` 声明工具，但工具实现将迁移为**客户端（WPS 插件）JS 脚本**执行（不再要求客户机有 Python 环境，也不在后端执行脚本）。
