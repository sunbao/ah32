...
## 2026-03-22 Bidding Live Story Findings
- `bidding-helper` live-fixture story ???????????`/agentic/chat/stream` ?? `phase=start` ????? heartbeat??? `plan/content/done`?
- ?????????????
  - Plan fast-path ??????????
  - ???? dedicated plan model?????????
  - ?????????? heartbeat ????????????????? auto-repair?
- ???????????????????????????????????
  1. ?? `bidding-helper` skill manifest ?????`SkillRegistry` ? `Extra data`??????????
  2. manifest ?? `delivery.default_writeback=direct`??????????????? Plan JSON????????? chat?
  3. manifest ?? `active_doc_text` ???????????????????? prompt?
  4. `_is_chat_only_intent()` ???????????????????????????????
- ???????????
  - ???????????
    - ???? `ReActAgent.stream_chat(...)`??????????? `thinking: ??????? Plan...`?
    - ??????????????????
  - ?? HTTP ???
...
- ????????
  - **???**?
    - `bidding-helper` live story ????? heartbeat?
    - ??????????
    - ??????????
    - HTTP ??????? plan ?????????? plan?
  - **??????**?
    - ?? plan ??????????????? live-story bench?
    - ????????????? plan ???? -> auto-repair ?? plan????? 89 ??

## 2026-03-23 Bidding Live Story Findings
- 这轮确认的关键事实不是“模型终于自己好了”，而是我们修掉了两个很具体的系统问题：
  1. plan 超时门槛太激进：75 秒会把本来能返回的 live-fixture plan 直接切掉；
  2. Writer 表格类 Plan 的语义修复缺口：模型会生成 `insert_table + set_table_cell_text`，但经常漏 `row/col`，以前这类 Plan 会直接判死。
- 已修根因：
  - `src/ah32/agents/react_agent/core.py`
    - plan timeout 改为按 fast-path / repair 区分；
    - 纯 timeout 且无模型输出时不再重复打 repair；
    - writeback 诊断日志更细，能直接看到 timeout / invalid plan / repair success。
  - `src/ah32/plan/normalize.py`
    - 对 Writer `set_table_cell_text` 缺 `row/col` 的常见坏 Plan 做了确定性修复。
- 最关键的实测证据：
  - `policy_tender_live_v1` 之前的 repair 坏 Plan 已经被定位并修掉，最新可真实产出 `plan`；
  - `policy_bid_live_v1` 与 `policy_complaint_live_v1` 在同版后端上也都能持续产出 `plan`；
  - 当前三条 live story 的瓶颈已经从“功能性失败”降到了“延迟波动较大”。
- 当前残留风险：
  - `tender/bid` 的 plan 产出时间仍不稳定，最近观测范围大约在 `90s` 到 `200s+`；
  - 因此现在可以说“后端 live-fixture 可用了”，但还不能说“延迟已经收敛到稳定 benchmark 水平”。
